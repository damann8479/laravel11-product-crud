            ...
            <form action="{{ route('products.update', $product->id) }}" method="POST">
                ...
                <div>
                    ...
                    <input name="article_code" type="text"
                        ...
                        value="{{ old('article_code') ?? $product->article_code }}">
                    @error('article_code')
                ...
                <div class="mt-5">
                    ...
                    <textarea name="description" class="w-full border rounded-md focus:border-blue-500 p-2 outline-none">{{ old('description') ?? $product->description }}</textarea>
                    @error('description')
                        <p class="text-xs text-red-600 font-bold">{{ $message }}</p>
                    @enderror
                </div>
